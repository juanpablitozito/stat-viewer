...
async function carregarCarrossel() {
    try {
        ...
        // Remover overlay do slide
        // overlay.style.display = 'none';
        // Para remover o overlay, basta não incluir o elemento na estrutura do slide.
        // Remova o bloco <div class="overlay">...</div> do template abaixo:
        slidesContainer.innerHTML = filmes.map(filme => {
            const backdropUrl = filme.backdrop_path 
          ? `https://image.tmdb.org/t/p/w1280${filme.backdrop_path}`
          : 'https://via.placeholder.com/1280x500/333/666?text=Imagem+Indisponível';
           ...
           background-image: url('${backdropUrl}');
           background-size: cover; background-position: center;">
              <h2 style="color: white; text-shadow: 2px 2px 8px #000;">${filme.title}</h2>
              <p style="color: white; text-shadow: 1px 1px 6px #000;">${filme.overview ? filme.overview.substring(0, 150) + "..." : "Sem descrição disponível."}</p>
            </div>
            `;
        }).join('');
        total = filmes.length;
